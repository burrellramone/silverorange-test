...
final class PostImporter
{
     ...
     * @param string $dir The directory where the post files are located or the location of a specific post file
     *  to import
    ...
    public static function validatePost(array $post): bool
    {
        if (empty($post['id']) || empty($post['title']) || empty($post['body']) || empty($post['author_id'])) {
            return false;
        } elseif (empty($post['created_at']) || empty($post['modified_at'])) {
            return false;
        }
